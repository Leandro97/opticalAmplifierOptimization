...
public class ACOP_MOOProblem {
	...
	public ACOP_MOOProblem(AmplifierType type, ACOPCustomObject acopCO) {
		this.type 		  = type;
		this.acopCO 	  = acopCO;
		this.localProblem = ACOP_LOCAL_PROBLEM.AdGC;
	}

	public ACOP_MOOProblem(AmplifierType type, ACOPCustomObject acopCO, ACOP_LOCAL_PROBLEM localProblem) {
		this.type		  = type;
		this.acopCO		  = acopCO;
	...
	public double[] evaluate(float[] gains, float[] attenuations) {
		...
		int numberCh           = acopCO.getNumberOfChannels();
		float inputPowerCh     = acopCO.getInputPowerCh();
		SimulationSetup simSet = new SimSetAMPVOA(numberCh, inputPowerCh, 9.0f, acopCO.getNumberOfAmplifiers());
		double linkLength      = acopCO.getLinkLength();
...
//		OpticalSignal inputSignal = signal.createSignal();
		...
		ITUGridUniformSignal signal = new ITUGridUniformSignal(acopCO.getChannelType(), numberCh, acopCO.getInitialFerquency(), 1e11, -18, 50);
		OpticalSignal inputSignal   = signal.createSignal();
		
		if (acopCO.getChannelType().equals("random")) {
			inputSignal = signal.createRandomChannelSignal();
		}
		...
		if (gains == null) {
			...
			switch (localProblem) {
			case MaxGain:
				heuristic = new MaxGain(acopCO.getNumberOfAmplifiers(), simSet.getLINK_LOSSES(), inputSignal, function);
				heuristic.setSelectionOp(new MaxGainSelection());
				break;
			case AdGC:
				heuristic = new AdGC(acopCO.getNumberOfAmplifiers(), simSet.getLINK_LOSSES(), inputSignal, function);
			...
			voaAttenuation = (float) (amplifiers[amplifiers.length - 1].getOutputPower() - ampVoaAtt - totalInputPower
					- simSet.getROADM_ATT());
